...
from ipiqa.datasets.dataloader_utils import get_dataloaders
from ipiqa.common.dist_utils import main_process, is_main_process, is_dist_avail_and_initialized
...
class Trainer:
    ...
    def optimizer(self):
        if self._optimizer is None:
            # 可以用这个实现逐层lr decay
            # 需要重写model的get_optimizer_params,可以参考lavis
    ...
    def setup_output_dir(self):
        tag = self.config.run.get("tag", "")
        if tag:
            tag = f"_{tag}"
        output_dir = Path(self.config.run.output_dir) / f"{self.job_id}{tag}"
