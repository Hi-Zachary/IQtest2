"""MSQRNetV19 -- DG-MPQ+ (DASF) + DCAR.

调整/改进7.md (V19): 对已验证的 DG-MPQ 做低风险扩展 + degradation-conditioned
自适应回归。

Module 1 (DG-MPQ+): 原 DG-MPQ + DASF（deviation-aware scale fusion），
输出 delta_q 与显式 multi-level degradation profile。
Module 2 (DCAR): 用 degradation profile + global content 动态生成样本专属
回归权重，q = q_base + Δq（HyperNet 末层 zero-init，Full 初始 == B1）。

Quality forward:
    delta_q, deg_profile = dg_mpq_plus(H3,H6,H9,H12)
    v_q = v0_q + tanh(lambda_q)*delta_q
    h_q = quality_fusion([v_q, t_q]);  q_base = quality_head(h_q)
    if use_dcar: q = q_base + DCAR(global_v, h_q, deg_profile)

V8~V18 历史模块（cadr/qard/daps/ptlc/msrc/mscm/dg_mpq 原版 等）保留但不使用。
"""
...
from ipiqa.models.dg_mpq_plus import DgMpqPlus
from ipiqa.models.dcar import DCAR

from ipiqa.common.registry import registry


@registry.register_model("msqr_dgmpqplus_dcar_v19")
class MSQRNetV19(BaseModel):
    def __init__(
            self,
        ...
        self.dg_mpq = DgMpqPlus(width=self.backbone.visual_width, dim=dim,
                                use_dasf=use_dasf)                # always (DG-MPQ+)
        self.lambda_q = nn.Parameter(torch.tensor(self.outer_gate_init))
        self.dcar = DCAR(                        # always
    ...
    def forward(self, x, text):
        ...
        # ---------- Module 1: DG-MPQ+（含 DASF，输出 delta_q + degradation profile） ----------
        deg_profile = None
        if self.use_dg_mpq:
            delta_q, deg_profile = self.dg_mpq(hs[3], hs[6], hs[9], hs[12])
        ...
        # ---------- Module 2: DCAR（degradation-conditioned dynamic residual） ----------
        if self.use_dcar and deg_profile is not None:
            delta_pred = self.dcar(global_v, h_q, deg_profile)   # [B,1]
            q = q_base + delta_pred
            ratios["dcar_delta_ratio"] = (
    ...
    def get_gate_log(self):
        ...
        if self.use_dg_mpq:
            ...
            al = getattr(self.dg_mpq, "_last_alpha", None)
            if al is not None:
                for l in range(4):
                    out[f"alpha_{[3,6,9,12][l]}"] = round(al[:, l].mean().item(), 4)
        if self.use_dcar:
            st = getattr(self.dcar, "_last_stats", None)
            if st is not None:
                out.update({f"dcar_{k}": v for k, v in st.items()})
        return out
    ...
    def from_config(cls, cfg):
        return cls(
            ...
            use_dasf=cfg.get('use_dasf', True),
            use_dcar=cfg.get('use_dcar', True),
            dcar_hidden=cfg.get('dcar_hidden', 128),
