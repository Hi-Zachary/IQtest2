"""MSQRNetV23 -- MLPQ (Quality) + MGSC (Alignment) dual-branch framework.

调整/改进11.md:
    MLPQ: Multi-Level Patch Quality Modeling（多层 patch + learned weighting，无 deviation）
          = DgMpqAbl(use_multilevel=True, use_deviation=False)
    MGSC: Multi-Granularity Semantic Correspondence（全局 + 局部片段-patch 一致性）

    Q ← MLPQ (visual perceptual quality)
    A ← MGSC (semantic correspondence)

doublescore (AGIQA-3K)。V8~V22 历史模块保留不使用。
"""
...
from ipiqa.models.mgsc import MGSC

from ipiqa.common.registry import registry


@registry.register_model("msqr_mlpq_mgsc_v23")
class MSQRNetV23(BaseModel):
    def __init__(
            self,
        ...
        self.lambda_msc = nn.Parameter(torch.tensor(self.outer_gate_init))
    ...
    def forward(self, x, text):
        ...
        if self.use_mgsc:
            ...
            h_a = self.alignment_fusion(torch.cat([v_a, t_a, s_embed], dim=-1))
            a = self.align_head(h_a) + torch.tanh(self.lambda_msc) * s_align
            ratios["mgsc_align_ratio"] = (
                (torch.tanh(self.lambda_msc) * s_align).abs().mean()
                / (self.align_head(h_a).abs().mean() + 1e-6)
            ).item()
        else:
            s_embed = torch.zeros_like(v_a)
            h_a = self.alignment_fusion(torch.cat([v_a, t_a, s_embed], dim=-1))
    ...
    def get_gate_log(self):
        ...
        if self.use_mgsc:
            out["lambda_msc"] = torch.tanh(self.lambda_msc).item()
            st = getattr(self.mgsc, "_last_stats", None)
            if st is not None:
    ...
    def trainable_summary(self):
        ...
        return {
            ...
            "alignment_branch": cnt("alignment_") + cnt("lambda_msc"),
        }
