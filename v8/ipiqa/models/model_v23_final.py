"""MSQRNetV23Final -- Clean MLPQ + MGSCRefined（最终收敛版）。

调整/改进12.md Phase B + D:
    MLPQ = 干净版（多层级 patch + learned weighting，无 deviation 遗留）
    MGSC = MGSCRefined（删 beta/scalar residual）+ mgsc_mode 开关（global/local/global_local）

V23 / V23Refined 原版保留不使用。
"""
...
@registry.register_model("msqr_mlpq_mgsc_v23_final")
class MSQRNetV23Final(BaseModel):
    def __init__(
            self,
